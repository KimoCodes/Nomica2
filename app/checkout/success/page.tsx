...
export default function CheckoutSuccessPage() {
  ...
  const activate = useCallback(async () => {
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      if (activated) {
        // Set a grace-period cookie so subscription guard grants access
        document.cookie = `checkout_grace=1; path=/client; max-age=300; SameSite=Lax`;
        setStatus("success");
        return;
      }
    ...
    // Set grace cookie anyway and show success
    document.cookie = `checkout_grace=1; path=/client; max-age=300; SameSite=Lax`;
    setStatus("success");
  }, [sessionId]);
    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md">
        <CardContent className="flex flex-col items-center p-8 text-center">
          {status === "processing" ? (
            <>
              <Loader2 className="mb-4 size-12 text-primary animate-spin" />
